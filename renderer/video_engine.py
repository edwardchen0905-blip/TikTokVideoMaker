...
class VideoEngine:
    ...
    def generate(
        ...
        durations = template.get("durations", [duration]*len(images))
        if len(durations)!=len(images) or any(not math.isfinite(d) or not 1/30<=d<=600 for d in durations):
        ...
        if transition not in {'fade', 'slideleft', 'slideright', 'slideup', 'slidedown', 'none'}:
            raise ValueError("不支持的转场")
        if not math.isfinite(transition_duration) or not 0 <= transition_duration < min(durations):
        ...
        with tempfile.TemporaryDirectory(prefix="tvm_render_", dir=self.cache_root) as temporary:
            ...
            for index, image in enumerate(images, start=1):
                ...
                self._make_segment(image, segment, durations[index-1], template.get("motion", "none"))
            ...
            if len(segments) == 1:
                ...
                self._crossfade(segments, silent_video, durations, transition_duration, transition)

            if progress:
                progress("写入 MP4")
            completed = work / 'completed.mp4'
            if music_path:
                self._add_music(silent_video, music_path, completed, float(template.get("music_volume", 0.65)), template.get("music_fade", 0), template.get("expected_duration", sum(durations)-(len(images)-1)*transition_duration), template.get("music_playback", "loop"))
            else:
    ...
    def _make_segment(self, image: Path, output: Path, duration: float, motion: str) -> None:
        ...
        size = '980:1740' if motion in {'zoom','pan'} else '1080:1920'
        video_filter = f"scale={size}:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2:color=white,setsar=1"
        if motion == 'zoom':
            frames=max(1,round(duration*30))
            video_filter += f",zoompan=z='1+0.08*min(on/{frames},1)':x='iw/2-iw/zoom/2':y='ih/2-ih/zoom/2':d={1 if is_video else frames}:s=1080x1920:fps=30"
        elif motion == 'pan':
            frames=max(1,round(duration*30))
            video_filter += f",zoompan=z=1.04:x='iw/2-iw/zoom/2+10*sin(2*PI*on/{frames})':y='ih/2-ih/zoom/2':d={1 if is_video else frames}:s=1080x1920:fps=30"
        elif motion != 'none':
    ...
    def _crossfade(
        ...
        for index, segment in enumerate(segments[1:], 1):
            ...
            self._run([self.ffmpeg,'-hide_banner','-loglevel','error','-y','-i',str(previous),'-i',str(segment),
                ...
                f'[0:v][1:v]xfade=transition={transition}:duration={transition_duration}:offset={elapsed-transition_duration}[v]',
            ...
            previous=target
            elapsed+=duration[index]-transition_duration
...
def validate_output(path, config):
    ...
    if not video or video.get('codec_name')!='h264' or (video.get('width'),video.get('height'))!=(1080,1920):raise ValueError('输出编码或分辨率不符合任务')
    ...
    return {'status':'passed','full_decode':True,'width':1080,'height':1920,'fps':30,'codec':'h264','duration':actual,'expected_duration':expected,'audio':bool(audio)}
