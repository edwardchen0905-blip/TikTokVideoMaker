...
const evidence=path.join(__dirname,'..','build','evidence');let failurePage;
(async()=>{
 ...
 assert.equal(s.videos.length,5);assert.equal(s.videos.length,expected.state.videos.length);assert.ok(s.tasks.every(t=>t.status==='done'));assert.equal(s.publications[0].status,'pending');
 ...
 assert.deepEqual([...new Set(s.tasks.map(t=>JSON.parse(t.config).resolution))].sort(),['1080x1080','1080x1920','1920x1080']);
 assert.ok(s.tasks.every(t=>JSON.parse(t.config).fit==='contain'&&t.template===''));
 assert.ok(s.tasks.some(t=>JSON.parse(t.config).music_snapshot?.Music_ID));
 ...
 fs.writeFileSync(path.join(evidence,'windows-restart.json'),JSON.stringify({passed:true,records_equal:true,files_unchanged:true,originals_protected:true,text_and_music_trace_preserved:true,resolutions:['1080x1920','1080x1080','1920x1080'],played_videos:s.videos.map(v=>v.id)},null,2));
 console.log('ORIGINAL_EXE_RESTART_PASSED');process.exit(0);
})().catch(async e=>{
 const failure={error:String(e)};
 try{if(failurePage&&!failurePage.isClosed())await failurePage.screenshot({path:path.join(evidence,'windows-restart-failure.png'),fullPage:true})}catch(screenshotError){failure.screenshot_error=String(screenshotError)}
 fs.writeFileSync(path.join(evidence,'windows-restart-failure.json'),JSON.stringify(failure,null,2));
